...
def test_warning_log_rule_flags_warning_logs_as_medium():
    rule = WarningLogRule({})
    data = [_entry("w"), _entry("e"), _entry("f")]
...
def test_warning_log_rule_no_issue_without_warnings():
    rule = WarningLogRule({})
    data = [_entry("e"), _entry("f")]
    test_results, _ = rule.apply(data, extra_info={"host": "test-host"})
    assert test_results == []


def test_warning_log_rule_counts_warnings():
    rule = WarningLogRule({})
    data = [_entry("w"), _entry("w"), _entry("e")]
    test_results, _ = rule.apply(data, extra_info={"host": "test-host"})
    assert len(test_results) == 1
